...
# СВІТЛО⚡БЕЗПЕКА (v1.4.11 Autonomous Edition)
...
- **Telegram Notifications:** Покращено наповненість сповіщень — тепер «Точність» та «За графіком» відображаються навіть при значних відхиленнях (ліміт розширено до 3 годин).
- **Точність даних:** Виправлено розрахунок відхилень — тепер він базується на фактичному часі події, а не часі виявлення.
- **Docker Fix:** Усунено помилку `Invalid cross-device link` при генерації чартів.
