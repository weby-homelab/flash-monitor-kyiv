...
```text
.
├── app.py                   # Основний Flask додаток (API та Dashboard)
├── light_service.py         # Логіка обробки подій світла та таймаутів
├── run_background.py        # Пускач фонових сервісів (Monitor + Scheduler)
├── generate_daily_report.py  # Генератор денних графіків (Actual vs Plan)
├── generate_weekly_report.py # Генератор тижневої статистики
├── generate_text_report.py   # Формування текстових графіків для Telegram
├── config.json              # Конфігурація UI, іконок та джерел даних
├── dashboard_preview.jpg    # Скріншот інтерфейсу для документації
├── Dockerfile               # Інструкція збірки Docker-образу
├── docker-compose.yml       # Сценарій розгортання в контейнерах
├── start.sh                 # Скрипт швидкого запуску (Gunicorn)
├── static/                  # Статичні файли (CSS, JS, іконки, згенеровані PNG)
├── templates/               # HTML-шаблони (index.html)
└── data/                    # (Volume) Сховище JSON-файлів стану та логів
```
