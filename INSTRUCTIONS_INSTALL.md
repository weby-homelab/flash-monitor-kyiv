...
Приклад конфігурації для Києва (вказавши свою групу відключень, наприклад `"1"`, `"2"` або `"3"`):
```json
{
  "settings": {
    "region": "kyiv",
...
1. Дізнайтеся ваш автоматично згенерований секретний ключ (файл створиться після першого запуску):
   ```bash
